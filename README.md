...
9. **Buttons:** BACK always goes back one step, down to the start screen, and only there does it leave the app. During a nap, the Stay Awake guard and the alarm nothing stops by accident: press **BACK twice** within 4 seconds to stop and go back to the start screen (no summary; the Stay Awake doze alarm goes back on guard instead), or **START twice** to stop and see your stats. On the summary one BACK goes back to the start screen. On the start screen press **BACK twice** to leave the app: the first press shows "Press BACK again to exit". The first BACK on any screen, and the first START on the alarm, show a popup saying what the second press does ("Press BACK again to end nap", "... to stop", "Press START again to stop") and the footer repeats it in red; the first START during the nap shows the peek card, whose footer says "START again: stop + stats". A BACK and a START never combine into a pair, and a first press made before the alarm started does not count on the alarm screen. Screen taps and swipes are ignored during a nap and during the alarm, so a wrist or sleeve on the pillow cannot silence anything; outside a nap a right swipe works like BACK. For 1.5 seconds after a stop every press is ignored (never extended by further presses), so extra presses of a half-asleep hand cannot skip the summary or run on into the next screen, and you can never be trapped in a screen.
...
- stopping the doze alarm (START or BACK twice) puts the watch back on guard, so one session catches every doze;
- START twice on the guard screen ends the session with its summary (how long you stayed awake and how many dozes were caught); BACK twice goes back to the start screen.
...
source/
  ...
  ConfirmPress.mc               Two-press confirmation: BACK x2 goes back (exits on the start screen), START x2 stops; the keys never pair
