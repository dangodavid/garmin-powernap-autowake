...
9. **Buttons:** BACK is a normal back button: one press goes back one step, down to the start screen, and only there does it leave the app. During a nap, the Stay Awake guard and the alarm, **BACK** stops and goes back to the start screen (no summary; the Stay Awake doze alarm goes back on guard instead), and **START twice** within 4 seconds stops and shows your stats. On the summary one BACK goes back to the start screen. On the start screen press **BACK twice** to leave the app: the first press shows "Press BACK again to exit". The first START on the alarm shows "Press START again to stop" and the footer repeats it in red; the first START during the nap shows the peek card, whose footer says "START again: stop + stats". A BACK never completes a START pair, and a first START made before the alarm started does not count on the alarm screen. BACK itself is not protected against an accidental press during the nap: one press ends the nap and its alarm. Screen taps and swipes are ignored during a nap and during the alarm, so a wrist or sleeve on the pillow cannot silence anything; outside a nap a right swipe works like BACK. For 1.5 seconds after a stop every press is ignored (never extended by further presses), so extra presses of a half-asleep hand cannot skip the summary or run on into the next screen, and you can never be trapped in a screen.
10. **Summary screen:** Shows time asleep, how long it took you to fall asleep ("Fell asleep in 10 min"), how much of the planned nap was completed (ring), the number of wake episodes, average and minimum HR, and the time window you slept. START sets up a new nap; BACK goes back to the start screen.
...
- stopping the doze alarm (one BACK, or START twice) puts the watch back on guard, so one session catches every doze;
- START twice on the guard screen ends the session with its summary (how long you stayed awake and how many dozes were caught); one BACK goes back to the start screen.
...
- In a moving vehicle (train, car, plane) the vibrations count as movement, so sleep is usually not detected; the alarm still rings at the "Alarm by" time.
